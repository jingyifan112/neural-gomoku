...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Audit hard-guard tensor/loss/gradient semantics for b4c64 no-save guard-aware route."
    )
    ap.add_argument(
        "--dataset",
        type=Path,
...
def main() -> None:
    ...
    loss, terms, tb = build_terms(model, reference_model, hard_samples, tensors, beats_mask, args)
    ...
    loss2, _terms2, _tb2 = build_terms(model, reference_model, hard_samples, tensors, beats_mask, args)
    loss2.backward()
    ...
    summary = {
        "scope": "hard guard tensor/gradient semantics audit only; no checkpoint save; no C export; no public benchmark; no promotion",
        ...
        "kl_masked_softmax_should_be_near_zero_when_model_equals_reference": terms_float["reference_kl_masked_softmax"],
        "kl_from_log_should_be_near_zero_when_model_equals_reference": terms_float["reference_kl_from_log"],
    ...
    (args.out_dir / "hardguard_gradient_semantics_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")

    lines = []
    lines.append("# b4c64 hard guard tensor/gradient semantics audit")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    ...
    lines.append("")
    if summary["audit_findings"]["masked_softmax_kl_nonzero_with_identical_models"]:
        lines.append("- The reference KL path is suspect: `masked_softmax` KL is non-zero even when model and reference checkpoints are identical.")
        lines.append("- Next patch should replace KL reference probabilities with `masked_log_softmax(...).exp()` or otherwise make masked probability semantics consistent.")
    elif summary["audit_findings"]["any_target_grad_wrong_sign_for_ce_increase"]:
        lines.append("- Target CE gradient sign is suspect; inspect target action indexing and target/suppress tensor semantics.")
    else:
        lines.append("- KL/gradient signs look internally consistent; inspect optimizer step magnitude and interaction with policy head/shared logits.")
    lines.append("")
    ...
    for p in sorted(args.out_dir.glob("*")):
        if p.is_file():
            lines.append(f"- `{p}`")
    lines.append("")
    (args.out_dir / "hardguard_gradient_semantics_report.md").write_text("\n".join(lines))
