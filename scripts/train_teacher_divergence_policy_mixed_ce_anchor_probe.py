...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-checkpoint",
        type=Path,
        default=Path("checkpoints/15x15_teacher_divergence_policy_mixed_ce_anchor_probe.pt"),
    )
    parser.add_argument(
        "--eval-csv",
        type=Path,
        default=Path("analysis/integration_eval/teacher_divergence_policy_mixed_ce_anchor_probe_eval.csv"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("analysis/integration_eval/teacher_divergence_policy_mixed_ce_anchor_probe_report.md"),
    )
    ...
    parser.add_argument(
        ...
        default="train_teacher_divergence",
        help="Comma-separated splits to add as low-weight CE anchors.",
    )
    parser.add_argument(
        "--mixed-ce-anchor-label-types",
        default="",
        help="Optional comma-separated label_type filter for mixed CE anchor rows.",
    )
    parser.add_argument(
        "--mixed-ce-anchor-weight-scale",
        type=float,
        default=0.10,
        help="Multiplier applied to row weights for mixed CE anchor rows.",
    )
    parser.add_argument(
        "--mixed-ce-anchor-max-rows",
        type=int,
        default=0,
        help="Optional cap for mixed CE anchor rows. 0 means no cap.",
    )
...
def write_report(
    ...
    lines: list[str] = []
    lines.append("# Teacher-divergence mixed-CE anchored policy probe report")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append("- Mixed-CE anchored policy-focused signal probe only.")
    lines.append("- Main cross-entropy trains `split == train_candidate` rows.")
    lines.append("- Low-weight mixed CE can also train selected anchor rows.")
