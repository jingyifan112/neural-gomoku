...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Evaluate policy-only rank/top-k gate for multi-suppress teacher-divergence rows."
    )
    ap.add_argument(
        "--dataset",
        type=Path,
    ...
    ap.add_argument("--model-a", type=Path, default=Path("checkpoints/15x15_current_best.pt"))
    ap.add_argument("--model-b", type=Path, default=Path("checkpoints/15x15_current_best.pt"))
    ap.add_argument("--margin", type=float, default=1.0)
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=Path("analysis/integration_eval/policy_only_rank_topk_gate_baseline_self_metrics.csv"),
    )
    ap.add_argument(
        "--out-report",
        type=Path,
        default=Path("analysis/integration_eval/policy_only_rank_topk_gate_baseline_self_report.md"),
    )
...
def score_policy(
    ...
    for action in suppress_actions:
        if legal_np[action] <= 0:
            raise ValueError(f"{sample['case_id']}: suppress illegal: {action_to_rc(action)}")
...
def compare_sample(
    ...
    before = score_policy(model_a, sample, device, margin)
    after = score_policy(model_b, sample, device, margin)
...
def load_anchor_count(path: Path) -> tuple[int, str]:
    try:
        anchors = load_anchor_samples(path)
        return len(anchors), "loaded"
...
def write_report(
    ...
    out: list[str] = []
    out += ["# Policy-only rank/top-k gate dry-run report", ""]
    out += ["## Scope", ""]
    out += ["- Evaluation only: no optimizer, no training, no checkpoint save."]
    out += ["- No C export, no public benchmark, no promotion."]
    ...
    out.append(verdict)
    out += ["", "## Decision", ""]
    out.append("Dry-run only. Do not train, export, public benchmark, or promote from this branch.")
    out.append("If this baseline self-check passes, the next step can compare a real candidate checkpoint with the same evaluator.")
    out.append("")
...
def main() -> int:
    ...
    anchor_count, anchor_status = load_anchor_count(args.anchor_snapshots)
    ...
    rows = [compare_sample(model_a, model_b, sample, device, args.margin) for sample in samples]
    ...
    print("rank_gt50_a:", summary["rank_gt50_a"], "rank_gt50_b:", summary["rank_gt50_b"], "delta:", delta(summary, "rank_gt50"))
    ...
    print("dry-run only; no training/checkpoint/export/benchmark/promotion")
    return 0
