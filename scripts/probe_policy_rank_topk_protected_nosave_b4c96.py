...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "b4c96-safe no-save protected rank/top-k objective ablation wrapper. "
    ...
    ap.add_argument(
        ...
        type=Path,
        default=Path(
            "analysis/integration_eval/b4c96_safe_nosave_ablation_wrapper/"
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.init_checkpoint.name == "15x15_current_best.pt":
        raise ValueError("refusing to use current_best as b4c96 init checkpoint")
    if args.reference_checkpoint.name == "15x15_current_best.pt":
        raise ValueError("refusing to use current_best as b4c96 reference checkpoint")


def load_model_b4c96(
    checkpoint: Path,
    device: torch.device,
    board_size: int,
    win_length: int,
...
def verdict(group_rows: list[dict[str, Any]]) -> str:
    ...
    if train_ok and protected_ok and tail_ok:
        return "PASS_B4C96_SAFE_NO_SAVE_PROBE"
    return "FAIL_B4C96_SAFE_NO_SAVE_PROBE"
...
def write_report(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines += ["# b4c96-safe protected no-save objective ablation wrapper report", ""]
    lines += ["## Scope", ""]
    lines += [
...
def main() -> int:
    ...
    reference_model = load_model_b4c96(
    ...
    print("b4c96-safe no-save probe complete; no checkpoint saved")
    return 0
