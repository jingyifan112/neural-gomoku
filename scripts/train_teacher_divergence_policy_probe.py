...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Small policy-focused teacher-divergence probe. "
    ...
    parser.add_argument(
        "--out-checkpoint",
        type=Path,
        default=Path("checkpoints/15x15_teacher_divergence_policy_probe.pt"),
    )
    parser.add_argument(
        "--eval-csv",
        type=Path,
        default=Path("analysis/integration_eval/teacher_divergence_policy_probe_eval.csv"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("analysis/integration_eval/teacher_divergence_policy_probe_report.md"),
    )
...
def write_report(
    ...
    lines: list[str] = []
    lines.append("# Teacher-divergence policy probe report")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append("- Small policy-focused signal probe only.")
    lines.append("- Trains only `split == train_candidate` rows.")
