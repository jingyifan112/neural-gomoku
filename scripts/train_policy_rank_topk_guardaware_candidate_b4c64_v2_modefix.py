...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out-candidate-metadata", type=Path, required=True)
...
def main() -> int:
    ...
    # saved_candidate_route_v1:
    # This copied script is allowed to save a candidate only after the same
    # wrapper gate verdict passes. It must not overwrite current-best.
    if not str(args.out_checkpoint).startswith("checkpoints/probes/"):
        raise ValueError(f"candidate checkpoint must be under checkpoints/probes/: {args.out_checkpoint}")
    if args.out_checkpoint.name in {
        "15x15_current_best.pt",
        "15x15_current_best_margin_g2m13_m15_3pair_b.pt",
    }:
        raise ValueError(f"refusing current-best-like checkpoint name: {args.out_checkpoint}")
    if not str(final_verdict).startswith("PASS"):
        raise RuntimeError(f"refusing to save candidate because verdict is not PASS: {final_verdict}")

    args.out_checkpoint.parent.mkdir(parents=True, exist_ok=True)
    args.out_candidate_metadata.parent.mkdir(parents=True, exist_ok=True)

    torch.save(model.state_dict(), args.out_checkpoint)

    candidate_metadata = {
        "scope": "saved candidate from b4c64 guard-aware v2 modefix route; not promotion; not current-best overwrite; no C export; no public benchmark run in this script",
        ...
        "hard_guard_reference_kl_weight": args.hard_guard_reference_kl_weight,
        "hard_guard_target_ce_weight": args.hard_guard_target_ce_weight,
        "hard_guard_beats_weight": args.hard_guard_beats_weight,
        "hard_guard_beats_margin": args.hard_guard_beats_margin,
        "weight_decay": args.weight_decay,
        "seed": args.seed,
    }
    args.out_candidate_metadata.write_text(json.dumps(candidate_metadata, indent=2, sort_keys=True) + "\n")

    print("saved_candidate_route_v1 complete")
    print("out_checkpoint:", args.out_checkpoint)
    print("out_candidate_metadata:", args.out_candidate_metadata)
    print("candidate saved only after PASS verdict; current-best not overwritten")
    return 0
