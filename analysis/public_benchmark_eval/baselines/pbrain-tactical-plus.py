...
def pattern_score_for_player(x, y, p):
    ...
    for dx, dy in DIRS:
        ...
        if count >= 5:
            ...
            score = 8000
        elif count == 2 and opens == 1:
            score = 1000
        else:
            score = count * 20 + opens * 5
...
def move_score(x, y):
    # Player 1 is this engine, player 2 is opponent.
    self_score = pattern_score_for_player(x, y, 1)
    opp_score = pattern_score_for_player(x, y, 2)

    c = size // 2
    center_bonus = 40 - (abs(x - c) + abs(y - c))

    # Defense slightly preferred when opponent has an immediate/strong threat.
    return self_score + int(1.15 * opp_score) + center_bonus

def choose_move():
    moves = nearby_moves(radius=2)

    # deterministic tie-break: score, then center proximity, then coordinate
    c = size // 2
    best = None
    best_key = None
...
for raw in sys.stdin:
    ...
    try:
        if cmd == "ABOUT":
            print('name="tactical-plus", version="0.1", author="local"', flush=True)
